// services/admin.groups.service.ts - WITHOUT ENUMS
...
export class AdminGroupsService {
   ...
   */
// admin.groups.service.ts - Better error handling
...
static async analyzeGroupReports(groupId: string): Promise<ReportAnalysisResponse> {
  ...
  const promise = (async () => {
    try {
      ...
      // Remove from pending cache after 2 seconds even if failed
      setTimeout(() => {
        this.analyzeGroupReportsCache.delete(groupId);
      }, 2000);
    }
